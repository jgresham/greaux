...
const TOOLTIPS = {
  ...
  peMultiple:     "P/E applied to modeled 2029 GAAP net income. TSMC trades at lower multiples than US tech peers due to geopolitical risk premium (Taiwan Strait). USD ADS share price.",
...
function buildProjections(params) {
  ...
  const hpcShare2029   = total2029 > 0 ? revenue.hpc[FINAL_INDEX] / total2029 * 100 : 0;

  return { revenue, earnings, total2024, total2029, earnings2029, marketCap, sharePrice, eps2029, cagr5y, hpcShare2029 };
}
  ...
  return (
    <div style={{ position: 'relative', width: '100%', height: type === 'earnings' ? 310 : 360 }}>
      <canvas ref={canvasRef} role="img"
        aria-label={`Stacked bar chart of TSMC ${type} by platform 2022 through 2029`}>
        TSMC {type} by platform, 2022–2024 actual and 2025–2029 projected.
      </canvas>
    </div>
  );
}
      ...
      <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: 14 }}>
              <div>
                ...
                <p style={{ fontSize: 14, color: 'var(--text3)', fontFamily: 'var(--mono)', maxWidth: 720, lineHeight: 1.6 }}>
                  2022–2024 actuals · Projected 2025–2029 · USD billions · Calendar year · ADS share price
                </p>
              </div>
        ...
        <main style={{ flex: 1, padding: '2rem 2.5rem', maxWidth: 1100, margin: '0 auto', width: '100%' }}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 12, marginBottom: '1.5rem' }}>
            ...
            <MetricCard label="HPC Share 2029"  value={`${fmt(projections.hpcShare2029, 0)}%`} sub="of 2029 revenue"                   accent={SEGMENT_COLORS.hpc} />
            <MetricCard label="2029 ADS Price"  value={fmtUsd(projections.sharePrice)}      sub={`EPS ${fmtUsd(projections.eps2029)}`} accent={ACCENT} />
          </div>
          ...
          <section style={{ background: 'linear-gradient(135deg, var(--bg2) 0%, rgba(230,0,18,0.05) 100%)', border: '1px solid rgba(230,0,18,0.28)', borderRadius: 'var(--radius-lg)', padding: '1.6rem 2rem', marginBottom: '1.5rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 20 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6, fontSize: 12, fontFamily: 'var(--mono)', color: 'var(--text3)', textAlign: 'right' }}>
              ...
              <span>{fmt(projections.hpcShare2029, 0)}% of 2029 revenue from HPC</span>
