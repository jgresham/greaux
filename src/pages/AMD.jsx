...
const TOOLTIPS = {
  ...
  peMultiple: 'P/E multiple applied to modeled 2030 non-GAAP net income.',
...
function buildProjections(params) {
  ...
  return { revenue, earnings, total2024, total2030, earnings2030, marketCap, sharePrice, eps2030, cagr6y, dcShare2030 };
}
  ...
  return (
    <div style={{ position: 'relative', width: '100%', height: type === 'earnings' ? 310 : 360 }}>
      <canvas ref={canvasRef} role="img"
        aria-label={`Stacked bar chart of AMD ${type} by segment 2022 through 2030`}>
        AMD {type} by segment, 2022–2024 actual and 2025–2030 projected.
      </canvas>
    </div>
  );
}
      ...
      <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
        ...
        <main style={{ flex: 1, padding: '2rem 2.5rem', maxWidth: 1100, margin: '0 auto', width: '100%' }}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 12, marginBottom: '1.5rem' }}>
            ...
            <MetricCard label="2030 Earnings"  value={fmtUsdB(projections.earnings2030)} sub={`${params.netMargin}% non-GAAP margin`} accent="var(--green)" />
            <MetricCard label="2030 Valuation" value={fmtUsdB(projections.marketCap)}    sub={`${params.peMultiple}x P/E`}     accent="var(--pink)" />
            <MetricCard label="DC Share 2030"  value={`${fmt(projections.dcShare2030, 0)}%`} sub="of 2030 revenue"             accent={SEGMENT_COLORS.dataCenter} />
          ...
          <section style={{ background: 'linear-gradient(135deg, var(--bg2) 0%, rgba(237,28,36,0.05) 100%)', border: '1px solid rgba(237,28,36,0.28)', borderRadius: 'var(--radius-lg)', padding: '1.6rem 2rem', marginBottom: '1.5rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 20 }}>
            <div>
              ...
              <div style={{ display: 'flex', gap: 20, flexWrap: 'wrap', fontSize: 12, fontFamily: 'var(--mono)', color: 'var(--text3)' }}>
                ...
                <span>{fmtUsdB(projections.earnings2030)} non-GAAP net income</span>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6, fontSize: 12, fontFamily: 'var(--mono)', color: 'var(--text3)', textAlign: 'right' }}>
              ...
              <span>{fmt(projections.dcShare2030, 0)}% of 2030 revenue from Data Center</span>
